...
void center_text(int width, char *text)
{
	...
	// If the len of text is less than console width then
...
void center_text(int width, char *text)
{
	...
	// Reason: The middle character of the text
	// should be at the center of the width
	// num_of_spaces = width/2 - len(text)/2 = (width-len(text))/2
	int num_of_spaces = (width - txt_len) / 2;
